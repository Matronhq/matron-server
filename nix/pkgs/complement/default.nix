...
let
  ...
  start = writeShellScriptBin "start" ''
    ...
    ${lib.getExe' coreutils "env"} \
      MATRON_SERVER_SERVER_NAME="$SERVER_NAME" \
      ${lib.getExe main'}
  '';
in

dockerTools.buildImage {
  name = "complement-matron-server";
  tag = "main";
  ...
  config = {
    ...
    Env = [
      "MATRON_SERVER_TLS__KEY=${./private_key.key}"
      "MATRON_SERVER_TLS__CERTS=${./certificate.crt}"
      "MATRON_SERVER_CONFIG=${./config.toml}"
      "RUST_BACKTRACE=full"
    ];
