...
use matron_server_core::{Result, pdu::PduBuilder};
...
///
/// Users in this room are considered admins by matron-server, and the room can be
	...
	services
		...
		.build_and_append_pdu(
			PduBuilder::state(String::new(), &RoomTopicEventContent {
				...
				topic: format!("Manage {} | Run commands prefixed with `!admin` | Run `!admin -h` for help | Documentation: https://matronhq.github.io/matron-server", services.config.server_name),
			}),
			server_user,
			&room_id,
			&state_lock,
