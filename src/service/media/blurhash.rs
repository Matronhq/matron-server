...
use matron_server_core::config::BlurhashConfig as CoreBlurhashConfig;
...
pub fn create_blurhash(
	...
	matron_server_core::debug_warn!("blurhashing on upload support was not compiled");
...
pub fn create_blurhash(
	...
	get_blurhash_from_request(file, content_type, file_name, config)
		.map_err(|e| matron_server_core::err!(debug_error!("blurhashing error: {e}")))
		.map(Some)
}
