...
) -> Result<T, matron_server_core::Error> {
	r.map_or_else(or_else, and_then)
}

#[inline(always)]
pub(crate) fn and_then<T>(t: T) -> Result<T, matron_server_core::Error> { Ok(t) }

pub(crate) fn or_else<T>(e: rocksdb::Error) -> Result<T, matron_server_core::Error> { Err(map_err(e)) }

#[inline]
pub(crate) fn is_incomplete(e: &rocksdb::Error) -> bool { e.kind() == ErrorKind::Incomplete }

pub(crate) fn map_err(e: rocksdb::Error) -> matron_server_core::Error {
