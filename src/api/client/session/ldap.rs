...
use matron_server_core::{Err, Result, debug};
...
pub(super) async fn ldap_login(
	...
	// password is reserved for deactivated accounts. The matron-server password field
	...
	if !services.config.ldap.admin_filter.is_empty() {
		let is_matron_server_admin = services
			.admin
			.user_is_admin(lowercased_user_id)
			.await;

		if is_ldap_admin && !is_matron_server_admin {
			services
				.admin
				.make_user_admin(lowercased_user_id)
				.boxed()
				.await?;
		} else if !is_ldap_admin && is_matron_server_admin {
			services
				.admin
