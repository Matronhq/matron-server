...
When developing in debug-builds with the nightly toolchain, Matron Server is modular
...
linkers, and please let us know in the [Matron Server Matrix room][7] the linker
...
0. Have patience. Don't hesitate to join the [Matron Server Matrix room][7] to
...
1. Install the nightly toolchain using rustup. You may need to use `rustup
   override set nightly` in your local Matron Server directory, or use `cargo
...
9. In your Matron Server server terminal, hit/send `CTRL+C` signal. This will tell
   Matron Server to find which libraries need to be reloaded, and reloads them as
   necessary.
...
To shutdown Matron Server in this setup, hit/send `CTRL+\`. Normal builds still
...
the [Matron Server Matrix room][7] the problem. mold can be installed typically
...
![Matron Server's dynamic library setup diagram - created by Jason
...
![Matron Server's reload and load order diagram - created by Jason
...
[7]: https://matrix.to/#/#matron:matron.chat
