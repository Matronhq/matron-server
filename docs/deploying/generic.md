...
> [!TIP]
>
> Getting help: If you run into any problems while setting up Matron Server
> [open an issue on GitHub](https://github.com/matronhq/matron-server/issues/new).

## Installing Matron Server
...
release [here](https://github.com/matronhq/matron-server/releases/latest) or
...
Nix to build matron-server as this has the most
...
`result/bin/matron-server`. This is reproducible with the static binaries produced
in our CI.
...
You can build Matron Server using `cargo build --release --all-features`

## Adding a Matron Server user

While Matron Server can run as any user it is better to use dedicated users for
different services. This also allows you to make sure that the file permissions
are correctly set up.

In Debian, you can use this command to create a Matron Server user:

```bash
sudo adduser --system matron-server --group --disabled-login --no-create-home
```

For distros without `adduser` (or where it's a symlink to `useradd`):

```bash
sudo useradd -r --shell /usr/bin/nologin --no-create-home matron-server
```
...
delegation. Matron Server has config options for doing delegation, or you can configure
...
If Matron Server runs behind a router or in a container and has a different public
...
Two example systemd units for Matron Server can be found
[on the configuration page](../configuration/examples.md#debian-systemd-unit-file).
You may need to change the `ExecStart=` path to where you placed the Matron Server
binary if it is not `/usr/bin/matron-server`.
...
configured default `/var/lib/matron-server`, you need to add your path to the
systemd unit's `ReadWritePaths=`. This can be done by either directly editing
`matron-server.service` and reloading systemd, or running `systemctl edit matron-server.service`
...
## Creating the Matron Server configuration file

Now we need to create the Matron Server's config file in
`/etc/matron-server/matron-server.toml`. The example config can be found at
[matron-server-example.toml](../configuration/examples.md).
...
If you are using a dedicated user for Matron Server, you will need to allow it to
read the config. To do that you can run this:

```bash
sudo chown -R root:root /etc/matron-server
sudo chmod -R 755 /etc/matron-server
```

If you use the default database path you also need to run this:

```bash
sudo mkdir -p /var/lib/matron-server/
sudo chown -R matron-server:matron-server /var/lib/matron-server/
sudo chmod 700 /var/lib/matron-server/
```
...
1. **Reverse proxy the following routes:**
   ...
   - `/_matron-server/` - ad-hoc Matron Server routes such as `/local_user_count` and `/server_version`

2. **Optionally reverse proxy (recommended):**
   - `/.well-known/matrix/client` and `/.well-known/matrix/server` if using Matron Server to perform delegation (see the `[global.well_known]` config section and the [delegation example](root-domain-delegation.md))
   - `/.well-known/matrix/support` if using Matron Server to send the homeserver admin contact and support page (formerly known as MSC1929)
   - `/` if you would like to see `hewwo from matron-server woof!` at the root
...
Now you can start Matron Server with:

```bash
sudo systemctl start matron-server
```

Set it to start automatically when your system boots with:

```bash
sudo systemctl enable matron-server
```
