...
This guide shows you how to configure Nginx as a reverse proxy for Matron Server with TLS support.
...
Create a new configuration file at `/etc/nginx/sites-available/matron-server` (or `/etc/nginx/conf.d/matron-server.conf` on some distributions):

```nginx
upstream matron-server {
  server 127.0.0.1:8008; # IP and port where matron-server is listening
}
...
server {
  ...
  # Increase this to match the max_request_size in your matron-server.toml
  client_max_body_size 100M;

  # Forward requests to Matron Server
  location / {
    proxy_pass http://matron-server;
...
- **`client_max_body_size`**: Must match or exceed `max_request_size` in your `matron-server.toml`
- **Do NOT use `$request_uri`** in `proxy_pass` - while some guides suggest this, it's not necessary for Matron Server and can cause issues
...
```bash
sudo ln -s /etc/nginx/sites-available/matron-server /etc/nginx/sites-enabled/
```
...
```bash
curl https://matrix.example.com/_matron-server/server_version
curl https://matrix.example.com:8448/_matron-server/server_version
```
...
Lighttpd has known issues with the `X-Matrix` authorization header, making federation non-functional. We do not recommend using Lighttpd with Matron Server.
