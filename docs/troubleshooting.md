...
> your Docker setup. We must remain focused on supporting Matron Server issues and
...
## Matron Server and Matrix issues

#### Lost access to admin room

You can reinvite yourself to the admin room through the following methods:
- Use the `--execute "users make_user_admin <username>"` Matron Server binary
argument once to invite yourself to the admin room on startup
- Use the Matron Server console/CLI to run the `users make_user_admin` command
...
properly functional when running Matron Server, resulting in federation issues. The
...
This is **not** a Matron Server issue, and is purely a Docker issue. It is not
...
Matron Server cannot provide a "works-for-everyone" Unbound DNS setup guide, but
...
in Matron Server config.

Raising `dns_cache_entries` in Matron Server config from the default can also assist
...
`matron-server --maintenance -O rocksdb_recovery_mode=0`
...
`matron-server --maintenance -O rocksdb_recovery_mode=1`
...
`matron-server --maintenance -O rocksdb_recovery_mode=2`
...
`matron-server --maintenance -O rocksdb_recovery_mode=3`
...
`matron-server --maintenance -O rocksdb_repair=true`
...
cd rocksdb
make -j$(nproc) ldb
./ldb repair --db=/var/lib/matron-server/ 2>./repair-log.txt
```
...
Matron Server builds without debug or trace log levels at compile time by default
...
Matron Server supports changing the tracing log environment filter on-the-fly using
...
Matron Server can ping other servers using `!admin debug ping <server>`. This takes
...
can see Matron Server's high-level allocator stats by using
...
which may only be visible in the Matron Server console CLI due to PDU size limits,
